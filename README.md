...
| Ejecutar pruebas            | `node tester.js --piece ./modules/<nombre>`                              |
...
- Exponer `runTests()` es opcional pero recomendado. Debe devolver `{ passed, failed, details }` para
  integrarse con `tester.js`.
...
modules/
  <nombre>/
    ...
    tests/            # (Opcional) Utilidades de test unitario/integración
```
...
- Toda pieza debe incluir un camino feliz de prueba: usa `tester.js` para validar `runTests()` o, en su
  defecto, el smoke test de microfrontends.
...
- Exportación de `runTests()` retornando estadísticas para el tester.
- Micro frontend accesible en `/microfrontends/queue-monitor.js` que consulta las métricas periódicamente.

## Checklist para nuevos módulos

1. Crea `./modules/<nombre>/index.js` exportando `start`, `metadata` y opcionalmente `runTests` y
   `microfrontend`.
...
6. Ejecuta `node tester.js --piece ./modules/<nombre>` para validar que las pruebas o el smoke test
   pasan.
